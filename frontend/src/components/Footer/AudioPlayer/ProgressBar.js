        ...
        <div className='bar'>
            <div
                ...
                style={{background: `linear-gradient(to right, #f35b04 ${percentListened}%, #1e1e24 0`}}>
                    <span
                        className={`progressKnob${audioState.currentTrack !== null ? '' : ' inactiveKnob'}`}
                        style={{left: `${percentListened - 2}%`}}
                    />
            </div>
            <span className={`barTime${audioState.currentTrack !== null ? '' : ' inactiveTime'}`}>{curTime || '00:00:00'} | {duration || '00:00:00'}</span>
        </div>
    )
}